...
router.post('/addBookByTitle', async(req, res) => {
    try {
        ...
        // Trouver API avec les meta d'un livre
        const existing = await Book.findOne({gutendexId: bookData.id});
        if (existing) { // Si le livre est dans la base de données : error
            return res.json({result: false, error: 'Book already in database'});
        }

        const authorName = (bookData.authors && bookData.authors[0] && bookData.authors[0].name) ? bookData.authors[0].name : 'Unknown';
        const synopsis = Array.isArray(bookData.summaries) ? bookData.summaries[0] : '';
        ...
        return res.json({result: true, data: saved});
    } catch (err) {
        console.error(err);
